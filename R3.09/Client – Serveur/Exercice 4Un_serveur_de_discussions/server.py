...
def handle_client(conn, address):
    ...
    while True:
        try:
            ...
            print(f"[{address[0]}] Client: {message}")
            ...
            if message.lower() == 'bye':
                ...
                broadcast(f"Serveur: Le serveur va s'arrêter.", conn)
                shutdown_server()
                return

            # Diffuser le message aux autres clients avec l'IP de l'expéditeur
            broadcast(f"[{address[0]}]: {message}", conn)
        except:
            break
